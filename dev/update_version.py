...
REPLACEMENT_RULES = [
    ReplacementRule(
        GF_INIT_PY_REL_PATH,
        GF_VERSION_PATTERN,
        lambda v, m: f"VERSION = {v.geniflow_repr()}",
    ),
    ReplacementRule(
        "docker-compose.yml",
        re.compile(r"(\$\{GF_VERSION:-)([\w.]+)(\})"),
...
def get_current_version() -> Version:
    version_text = GF_INIT_PY_PATH.read_text()

    match = re.search(GF_VERSION_PATTERN, version_text)
    if not match:
        raise RuntimeError(f"Failed to find version in {GF_INIT_PY_PATH}")

    return Version(int(match[1]), int(match[2]), int(match[3]), match[4], int(match[5]))


def main() -> None:
    parser = argparse.ArgumentParser(description="Bump GF version")
