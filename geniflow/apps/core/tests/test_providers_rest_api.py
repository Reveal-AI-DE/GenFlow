...
from geniflow.apps.core.tests.utils import enable_provider
from geniflow.apps.team.models import TeamRole
...
class ProviderTestCase(APITestCase):
    ...
    def setUpClass(cls):
        ...
        import geniflow.apps.ai.tests.register_providers  # noqa

        super().setUpClass()
        cls.client = APIClient()
        cls.admin_user, cls.regular_users = create_dummy_users(create_teams=True)
        cls.data = {"provider_name": "dummy", "encrypted_config": {"api_key": "test"}}
...
class ProviderCreateTestCase(ProviderTestCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.data = {"provider_name": "dummy", "credentials": {"api_key": "test"}}
...
class ProviderUpdateTestCase(ProviderTestCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.updated_data = {"provider_name": "dummy", "credentials": {"api_key": "updated_test"}}

    def update_provider(self, user, provider_id, data, team_id=None) -> HTTPResponse:
        url = (
            f"/api/providers/{provider_id}?team={team_id}"
            if team_id
            else f"/api/providers/{provider_id}"
        )
        with ForceLogin(user, self.client):
    ...
    def test_update_provider_invalid_credentials(self):
        team = self.regular_users[0]["teams"][0]["team"]
        provider = enable_provider(team=team, owner=self.admin_user, data=self.data)
        self.updated_data = {"provider_name": "dummy", "credentials": {}}
        response = self.update_provider(
            self.admin_user, provider.id, self.updated_data, team_id=team.id
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_delete_provider_user_forbidden(self):
        ...
        provider = enable_provider(team=team, owner=user, data=self.data)
        response = self.update_provider(
            another_user, provider.id, self.updated_data, team_id=team.id
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
