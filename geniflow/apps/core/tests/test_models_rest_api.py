...
class AIModelListTestCase(AIModelTestCase):
    def list_model(
        self, user, enabled_only: bool = False, model_type: str = None, team_id=None
    ) -> HTTPResponse:
        query_params = {}
        if team_id:
...
class AIModelRetrieveTestCase(AIModelTestCase):
    def get_model(
        self, user, model_id: str, provider_name: str = None, team_id=None
    ) -> HTTPResponse:
        query_params = {}
        if provider_name:
        ...
        url = (
            f"/api/models/{model_id}?{urlencode(query_params)}"
            if query_params
            else f"/api/models/{model_id}"
        )
        with ForceLogin(user, self.client):
            response = self.client.get(url)
        return response
...
class AIModelParameterConfigTestCase(AIModelTestCase):
    def get_parameter_config(
        ...
        url = (
            f"/api/models/{model_id}/parameter_config?{urlencode(query_params)}"
            if query_params
