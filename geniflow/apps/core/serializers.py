...
from geniflow.apps.ai import ai_provider_factory
from geniflow.apps.ai.base.entities.provider import CommonAIProviderEntity
from geniflow.apps.common.entities import ConfigurationEntity, FileEntity, TranslationEntity
from geniflow.apps.common.file_utils import create_media_symbolic_links
from geniflow.apps.common.security.encryptor import decrypt_token, encrypt_token
from geniflow.apps.common.storage import fs
from geniflow.apps.core.config.entities import AIProviderConfiguration, ModelWithProviderEntity
from geniflow.apps.core.config.provider_service import AIProviderConfigurationService
from geniflow.apps.core.models import CommonEntity, EntityGroup, Provider, ProviderModelConfig
from geniflow.apps.team.middleware import HttpRequestWithIamContext
from geniflow.apps.team.models import Team
...
class FileEntitySerializer(serializers.Serializer):
    ...
    def validate(self, data: dict) -> dict:
        ...
        if uploaded_file.size / (1024 * 1024) > settings.GF_LIMITS["MAX_FILE_SIZE"]:
            raise serializers.ValidationError(
                f"File size exceeds the limit of {settings.GF_LIMITS["MAX_FILE_SIZE"]} MB."
            )
        # check type
