...
from geniflow.apps.team.models import Membership, TeamRole
from geniflow.apps.team.serializers import MembershipReadSerializer
from geniflow.apps.team.tests.utils import ForceLogin, create_dummy_users
        ...
        for user in self.regular_users:
            for team_membership in user["teams"]:
                response = self.list_memberships(
                    self.admin_user, team_id=team_membership["team"].id
                )
                self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_retrieve_membership_admin(self):
        for user in self.regular_users:
            for team_membership in user["teams"]:
                ...
                self.check_response(
                    response, status.HTTP_200_OK, data=MembershipReadSerializer(membership).data
                )
    ...
    def test_update_membership_admin(self):
        for user in self.regular_users:
            for team_membership in user["teams"]:
                ...
                response = self.update_membership(
                    self.admin_user, membership.id, updated_membership
                )
    ...
    def test_update_membership_user(self):
        for user in self.regular_users:
            for team_membership in user["teams"]:
                ...
                if membership.is_active and membership.role in [
            ...
            for other_user in self.regular_users:
                if user["user"].id != other_user["user"].id:
                    for team_membership in other_user["teams"]:
                        ...
                        updated_membership["is_active"] = not membership.is_active
                        response = self.update_membership(
                            user["user"], membership.id, updated_membership
                        )
                        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        ...
        for user in self.regular_users:
            for team_membership in user["teams"]:
                if team_membership["membership"].is_active:
                    ...
                    updated_membership = MembershipReadSerializer(
                        team_membership["membership"]
                    ).data
                    updated_membership["role"] = TeamRole.OWNER.value
                    updated_membership["is_active"] = not team_membership["membership"].is_active
                    response = self.update_membership(
                        user["user"], team_membership["membership"].id, updated_membership
                    )
                    self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_delete_membership_user(self):
        for user in self.regular_users:
            for team_membership in user["teams"]:
                ...
                if membership.is_active and membership.role in [TeamRole.OWNER.value]:
                    self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
                    self.assertFalse(Membership.objects.filter(id=membership.id).exists())
                else:
                    self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        ...
        for user in self.regular_users:
            for team_membership in user["teams"]:
                if team_membership["membership"].is_active:
                    team_membership["membership"].role = TeamRole.MEMBER.value
                    team_membership["membership"].save()
                    response = self.delete_membership(
                        user["user"], team_membership["membership"].id
                    )
                    self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
                    self.assertTrue(
                        Membership.objects.filter(id=team_membership["membership"].id).exists()
                    )
