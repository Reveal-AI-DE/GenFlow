...
from geniflow.apps.team.models import Membership, Team
...
def get_membership(request, team):
    ...
    return Membership.objects.filter(team=team, user=request.user, is_active=True).first()


def build_iam_context(
    request: HttpRequestWithIamContext, team: Optional[Team], membership: Optional[Membership]
):
    """
    ...
    return {
        ...
        "team_owner_id": getattr(team.owner, "id", None) if team else None,
...
class GeniFlowBasePermission(metaclass=ABCMeta):
    """
    Abstract base class that defines the structure and behavior for permission handling in the GeniFlow application.
    """
    ...
    @classmethod
    @abstractmethod
    def create(cls, request, view, obj, iam_context) -> Sequence["GeniFlowBasePermission"]:
        """
...
class PermissionEnforcer(BasePermission):
    ...
    def check_permission(self, request, view, obj) -> bool:
        ...
        Iterates through all subclasses of GeniFlowBasePermission to check access.
        """
        ...
        iam_context = get_iam_context(request, obj)
        for perm_class in GeniFlowBasePermission.__subclasses__():
