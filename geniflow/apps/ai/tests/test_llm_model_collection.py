...
from geniflow.apps.ai.llm.messages import (
    AssistantMessage,
    SystemMessage,
    TextMessageContent,
    UserMessage,
...
class MessageTest(TestCase):
    ...
    def test_assistant_message_to_dict(self):
        ...
        self.assertEqual(
            ...
            {"content": "hello", "role": "assistant", "name": "assistant"},
        )
    ...
    def test_system_message_to_dict(self):
        ...
        self.assertEqual(
            ...
            {
                "content": [message_content1.model_dump(), message_content2.model_dump()],
                "role": "system",
            },
        )
...
class LLMModelCollectionTest(TestCase):
    ...
    def test_process_model_parameters(self):
        ...
        for model in models:
            ...
            processed_parameters = self.llm_model_collection._process_model_parameters(
                model.id, model_parameters
            )
    ...
    def test_process_model_parameters_use_template(self):
        ...
        for model in models:
            ...
            if model.parameter_configs:
                ...
                model_parameters[model.parameter_configs[1].name] = model.parameter_configs[
                    1
                ].default
                ...
                self.assertEqual(
                    processed_parameters[model.parameter_configs[0].name],
                    model.parameter_configs[1].default,
                )
    ...
    def test_process_model_parameters_validation_error(self):
        ...
        for model in models:
            ...
            if model.parameter_configs:
                ...
                self.assertEqual(
                    len(
                        self.llm_model_collection._process_model_parameters(
                            model.id, model_parameters
                        )
                    ),
                    0,
    ...
    def test_call(self):
        ...
        for model in models:
            ...
            input_count = self.llm_model_collection.get_tokens_count(model.id, messages)
            output_count = self.llm_model_collection.get_tokens_count(
                model.id, [self.llm_model_collection.RESPONSE]
            )
            ...
            if model.pricing is None:
                ...
                self.assertEqual(result.usage.total_tokens, input_count + output_count)
                total = (
                    input_count * model.pricing.unit * model.pricing.input
                    + output_count * model.pricing.unit * model.pricing.output
                )
