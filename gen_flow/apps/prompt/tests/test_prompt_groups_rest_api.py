...
class PromptGroupRetrieveTestCase(PromptGroupTestCase):
    ...
    def test_retrieve_prompt_group_admin(self):
        team = self.regular_users[0]['teams'][0]['team']
        response = self.retrieve_prompt_group(self.admin_user, group_id=self.prompt_group.id, team_id=team.id)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['name'], PROMPT_GROUP_DATA['name'])
    ...
    def test_retrieve_prompt_group_user_another_team(self):
        ...
        response = self.retrieve_prompt_group(another_user, group_id=self.prompt_group.id, team_id=team.id)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class PromptGroupDeleteTestCase(PromptGroupTestCase):
    ...
    def test_delete_prompt_group_admin(self):
        team = self.regular_users[0]['teams'][0]['team']
        response = self.delete_prompt_group(self.admin_user, group_id=self.prompt_group.id, team_id=team.id)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
    ...
    def test_delete_prompt_group_user_another_team(self):
        team = self.regular_users[0]['teams'][0]['team']
        another_user = self.regular_users[1]['user']
        response = self.delete_prompt_group(another_user, group_id=self.prompt_group.id, team_id=team.id)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class PromptGroupUpdateTestCase(PromptGroupTestCase):
    ...
    def test_update_prompt_group_admin_no_team(self):
        ...
        response = self.update_prompt_group(self.admin_user, group_id=self.prompt_group.id, data=new_data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_prompt_group_admin(self):
        ...
        response = self.update_prompt_group(self.admin_user, group_id=self.prompt_group.id, data=new_data, team_id=team.id)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['name'], new_data['name'])
    ...
    def test_update_prompt_group_user(self):
        ...
        response = self.update_prompt_group(user, group_id=self.prompt_group.id, data=new_data, team_id=team.id)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['name'], new_data['name'])
    ...
    def test_update_prompt_group_user_another_team(self):
        ...
        response = self.update_prompt_group(another_user, group_id=self.prompt_group.id, data=new_data, team_id=team.id)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
