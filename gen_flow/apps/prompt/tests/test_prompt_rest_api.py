...
from gen_flow.apps.prompt.tests.utils import  (create_prompt_group, create_prompt,
    PROMPT_GROUP_DATA, PROMPT_DATA, PROVIDER_DATA)
...
class PromptTestCase(APITestCase):
    ...
    def create_prompt(cls):
        ...
        cls.prompt = create_prompt(team=team, owner=user, data=data)
...
class PromptUpdateTestCase(PromptTestCase):
    ...
    def test_update_prompt_admin(self):
        ...
        response = self.update_prompt(self.admin_user, prompt_id=self.prompt.id, data=new_data, team_id=team.id)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['name'], new_data['name'])
    ...
    def test_update_prompt_user(self):
        ...
        response = self.update_prompt(user, prompt_id=self.prompt.id, data=new_data, team_id=team.id)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['name'], new_data['name'])
    ...
    def test_update_prompt_user_another_team(self):
        ...
        response = self.update_prompt(another_user, prompt_id=self.prompt.id, data=new_data, team_id=team.id)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
