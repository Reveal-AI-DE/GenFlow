...
COPY geniflow/requirements/ /tmp/geniflow/requirements/

ARG GF_CONFIGURATION="production"

RUN --mount=type=cache,target=/root/.cache/pip/http-v2 \
    python3 -m pip wheel --no-deps \
    -r /tmp/geniflow/requirements/${GF_CONFIGURATION}.txt \
    -w /tmp/wheelhouse
...
ARG USER="django"
ARG GF_CONFIGURATION="production"
ENV DJANGO_SETTINGS_MODULE="geniflow.settings.${GF_CONFIGURATION}"
...
COPY geniflow/nginx.conf /etc/nginx/nginx.conf
COPY --chown=${USER} supervisord/ ${HOME}/supervisord
COPY --chown=${USER} manage.py backend_entrypoint.sh wait-for-deps.sh ${HOME}/
COPY --chown=${USER} geniflow/ ${HOME}/geniflow
